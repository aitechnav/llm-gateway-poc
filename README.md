...
If the SentinelGuard Docker Hub image has not been published yet, build it from
the local SentinelGuard repo instead:

```bash
docker compose -f docker-compose.yml -f docker-compose.local-sentinelguard.yml up --build
```
...
If Docker Desktop is blocked by workplace licensing, run the same Compose stack
with Colima:
...
Then use the same `docker compose` commands from this README. If the official
SentinelGuard Docker Hub image is not published yet, use the local override:
